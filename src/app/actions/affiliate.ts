    ...
    try {
        ...
        clicksSnapshot.forEach(doc => {
            ...
            if (!campaignMap.has(campaignKey)) {
                campaignMap.set(campaignKey, { date: format(data.timestamp.toDate(), 'dd/MM/yy'), source, campaign, device, clicks: 0, registrations: 0 });
            }
        ...
        registrationsSnapshot.forEach(doc => {
            ...
            const campaignKey = `${dateStr}_${source}_${campaign}_${device}`;
             if (!campaignMap.has(campaignKey)) {
                campaignMap.set(campaignKey, { date: format(data.createdAt.toDate(), 'dd/MM/yy'), source, campaign, device, clicks: 0, registrations: 0 });
            }
